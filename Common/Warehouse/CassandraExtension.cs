...
public static class CassandraExtension {
    public static WebApplicationBuilder AddCassandra(this WebApplicationBuilder builder, ITypeDefinition[] configs) {
        ...
        builder.Services.AddSingleton<ICluster>(provider => {
            var host = builder.Configuration.GetValue("CassandraHost", "localhost");
            var port = builder.Configuration.GetValue("CassandraPort", 9042);
            var username = builder.Configuration.GetValue("CassandraUser", "cassandra");
            var password = builder.Configuration.GetValue("CassandraPass", "cassandra");
